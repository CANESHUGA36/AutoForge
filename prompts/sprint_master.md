...
功能组顺序：
- Tier 1 MVP（必须 ≥ 95% 通过）：F1 → F2 → F3 → F4
- Tier 2 核心体验（必须 ≥ 75% 通过）：F5 → F6 → F7 → F8 → F9
- Tier 3 扩展功能（必须 ≥ 65% 通过）：F10 → F11 → F12 → F13 → F14 → F15 → F16 → F17
- 设计标准：D
- 技术标准：T

**注意**：Harness 使用分层验证体系（代码审查 40% + 契约测试 35% + React DevTools 15% + 浏览器测试 10%）。Builder 的代码会被多层自动验证，你的 sprint.md 应指导 Builder 编写可被静态分析验证的代码。
