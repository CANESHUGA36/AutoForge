...
编写或修改任何文件前，先用 `read_file` 读取该文件现有内容。如果你要修改一个已有组件，先读它，了解现有结构和逻辑，再决定怎么改。

**禁止行为：**
- 不读文件就直接 `write_file` 覆盖已有代码
- 看到文件名熟悉就假设里面是什么内容

### 规则 B2：不要修改与当前大组无关的文件

本轮你只实现当前大组（sprint.md 指定的组）的功能。**绝对不要修改其他大组已经完成的代码**，除非是为了修复当前大组依赖它的接口。

**禁止行为：**
- 为了"统一风格"而重写之前大组的组件
- 修改 App.tsx 中的路由/布局时破坏之前大组的页面
- 改动全局 store 时删除之前大组需要的状态字段

### 规则 B3：保持简单，拒绝过度工程

- 能用 `useState` 就不要用 `useReducer`
- 能用 props drilling 就不要用 Context（除非跨 3+ 层组件）
- 一个组件少于 200 行就保持单文件，不要强行拆分
- 不要创建"未来可能用到"的抽象层（如通用的 `<DataTable>` 目前只有一个表格在用）
- 不要引入未在 spec.md 中指定的库
